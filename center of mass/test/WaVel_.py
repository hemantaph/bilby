...
class Fn:
	...
	def Y_(self, l, m, THE, PHI):
		...
		return sqrt(factorial(l+m)*factorial(l-m)*(2*l+1)/(4*pi*factorial(l+s)*factorial(l-s)))*np.sum((-1)**abs(l-k-s+m)*binom(l-s,k)*binom(l+s,k+s-m)*cos(THE/2)**(2*k+s-m)*sin(THE/2)**(2*l-2*k-s+m)*e**(1j*m*PHI))
