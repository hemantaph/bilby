...
cdef float complex C0(int l1, int l2, int m, float v_mag, float v2):
	cdef float hlp = 0.0
...
cdef float complex Cp(int l1, int l2, int m, float v_mag, float v0, float v1):
	cdef float hlp = 0.0
	cdef int k1, a1, b1, k2, a2, b2, u
				...
				for k2 in range(btz(m-2),l2-1):
					for a2 in range(2*l2-2*k2-m-1):
						for b2 in range(2*k2+m+3):
							...
							if abs(u%2) == 1:
								hlp += pow(-1,a1+a2)*G(l1,m+1,k1,a1,b1,l2,m,k2,a2,b2)*2*A(l1,m+1)/(u*(u*u-4))
							elif u == 2:
...
cdef float complex Cm(int l1, int l2, int m, float v_mag, float v0, float v1):
	cdef float hlp = 0.0
	cdef int k1, a1, b1, k2, a2, b2, u
	...
	for l1 in range(2,6):
		for l2 in range(2,6):
			for m in range(2*l2+1):
				...
				coeff[0][l1][l2][mhlp2] = C0(l1,l2,mhlp,v_mag,v2)
				coeff[1][l1][l2][mhlp2] = Cp(l1,l2,mhlp,v_mag,v0,v1)
				coeff[2][l1][l2][mhlp2] = Cm(l1,l2,mhlp,v_mag,v0,v1)
