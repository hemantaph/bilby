...
class Fn:
    ...
    def htilde(self):
        global iota,beta,D,et0,phic,tc,f,M,eta,chi,ff,delta
        ...
        psi_xi1_xipn_xi0_pc = self.psi_xi1_xipn_xi0_pc_
        psi_xi1_pc = self.psi_xi1_pc_
        psi_xi05_pc = self.psi_xi05_pc_
        ...
        #et0_values
        global et0_2,et0_3,et0_4,et0_5,et0_6

        #chi_values
        global chi_2b3,chi_4b3,chi_7,chi_19b3,chi_19b9,chi_22b3,chi_23b3,\
        chi_25b9,chi_28b9,chi_31b9,chi_38b9,chi_44b9,chi_47b9,chi_50b9,\
        chi_9b2,chi_19b6,chi_19b18,chi_23b6,chi_25b6,chi_29b6,chi_31b6,chi_31b18,\
        chi_37b18,chi_43b18,chi_49b18,chi_55b18,chi_95b18,chi_107b18,chi_113b18,\
        chi_119b18,chi_125b18,chi_131b18,chi_8,chi_25b3,chi_34b9,chi_37b9,chi_53b9,\
        chi_56b9

        #sinn_values
        global sin_i,sin_b,sin_2b,sin_3b,sin_4b

        #cosinne_values
        global cos_i,cos_b,cos_2b,cos_3b,cos_4b

        #log values
        global log_f,log_chi
        ...
        ################# x, 2D array ################### 
        global x,x_3b2,x_5b2,x_2,x_3,log_x
        x = np.ones((10,9))
        
        x[0,0] = Gmk * (np.abs(1 - (1 + 0)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,0] = Gmk * (np.abs(2 - (2 + 0)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,0] = Gmk * (np.abs(3 - (3 + 0)*k[2]/(1+k[2])))**(-n_2b3)
        x[3,0] = Gmk * (np.abs(4 - (4 + 0)*k[3]/(1+k[3])))**(-n_2b3)
        x[4,0] = Gmk * (np.abs(5 - (5 + 0)*k[4]/(1+k[4])))**(-n_2b3)
        x[5,0] = Gmk * (np.abs(6 - (6 + 0)*k[5]/(1+k[5])))**(-n_2b3)

        x[0,1] = Gmk * (np.abs(1 - (1 + 1)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,1] = Gmk * (np.abs(2 - (2 + 1)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,1] = Gmk * (np.abs(3 - (3 + 1)*k[2]/(1+k[2])))**(-n_2b3)
        x[3,1] = Gmk * (np.abs(4 - (4 + 1)*k[3]/(1+k[3])))**(-n_2b3)
        x[4,1] = Gmk * (np.abs(5 - (5 + 1)*k[4]/(1+k[4])))**(-n_2b3)
        
        x[0,2] = Gmk * (np.abs(1 - (1 + 2)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,2] = Gmk * (np.abs(2 - (2 + 2)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,2] = Gmk * (np.abs(3 - (3 + 2)*k[2]/(1+k[2])))**(-n_2b3)
        x[3,2] = Gmk * (np.abs(4 - (4 + 2)*k[3]/(1+k[3])))**(-n_2b3)
        
        x[0,3] = Gmk * (np.abs(1 - (1 + 3)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,3] = Gmk * (np.abs(2 - (2 + 3)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,3] = Gmk * (np.abs(3 - (3 + 3)*k[2]/(1+k[2])))**(-n_2b3)
        
        x[0,4] = Gmk * (np.abs(1 - (1 + 4)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,4] = Gmk * (np.abs(2 - (2 + 4)*k[1]/(1+k[1])))**(-n_2b3)
        
        x[0,-4] = Gmk * (np.abs(1 - (1 + -4)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,-4] = Gmk * (np.abs(2 - (2 + -4)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,-4] = Gmk * (np.abs(3 - (3 + -4)*k[2]/(1+k[2])))**(-n_2b3)
        x[3,-4] = Gmk * (np.abs(4 - (4 + -4)*k[3]/(1+k[3])))**(-n_2b3)
        x[4,-4] = Gmk * (np.abs(5 - (5 + -4)*k[4]/(1+k[4])))**(-n_2b3)
        x[5,-4] = Gmk * (np.abs(6 - (6 + -4)*k[5]/(1+k[5])))**(-n_2b3)
        x[6,-4] = Gmk * (np.abs(7 - (7 + -4)*k[6]/(1+k[6])))**(-n_2b3)
        x[7,-4] = Gmk * (np.abs(8 - (8 + -4)*k[7]/(1+k[7])))**(-n_2b3)
        x[8,-4] = Gmk * (np.abs(9 - (9 + -4)*k[8]/(1+k[8])))**(-n_2b3)
        x[9,-4] = Gmk * (np.abs(10 - (10 + -4)*k[9]/(1+k[9])))**(-n_2b3)
        
        x[0,-3] = Gmk * (np.abs(1 - (1 + -3)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,-3] = Gmk * (np.abs(2 - (2 + -3)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,-3] = Gmk * (np.abs(3 - (3 + -3)*k[2]/(1+k[2])))**(-n_2b3)
        x[3,-3] = Gmk * (np.abs(4 - (4 + -3)*k[3]/(1+k[3])))**(-n_2b3)
        x[4,-3] = Gmk * (np.abs(5 - (5 + -3)*k[4]/(1+k[4])))**(-n_2b3)
        x[5,-3] = Gmk * (np.abs(6 - (6 + -3)*k[5]/(1+k[5])))**(-n_2b3)
        x[6,-3] = Gmk * (np.abs(7 - (7 + -3)*k[6]/(1+k[6])))**(-n_2b3)
        x[7,-3] = Gmk * (np.abs(8 - (8 + -3)*k[7]/(1+k[7])))**(-n_2b3)
        x[8,-3] = Gmk * (np.abs(9 - (9 + -3)*k[8]/(1+k[8])))**(-n_2b3)
        
        x[0,-2] = Gmk * (np.abs(1 - (1 + -2)*k[0]/(1+k[0])))**(-n_2b3)
        x[1,-2] = Gmk * (np.abs(2 - (2 + -2)*k[1]/(1+k[1])))**(-n_2b3)
        x[2,-2] = Gmk * (np.abs(3 - (3 + -2)*k[2]/(1+k[2])))**(-n_2b3)
        x[3,-2] = Gmk * (np.abs(4 - (4 + -2)*k[3]/(1+k[3])))**(-n_2b3)
        x[4,-2] = Gmk * (np.abs(5 - (5 + -2)*k[4]/(1+k[4])))**(-n_2b3)
        x[5,-2] = Gmk * (np.abs(6 - (6 + -2)*k[5]/(1+k[5])))**(-n_2b3)
        x[6,-2] = Gmk * (np.abs(7 - (7 + -2)*k[6]/(1+k[6])))**(-n_2b3)
        x[7,-2] = Gmk * (np.abs(8 - (8 + -2)*k[7]/(1+k[7])))**(-n_2b3)     
        ...
        x[3,-1] = Gmk * (np.abs(4 - (4 + -1)*k[3]/(1+k[3])))**(-n_2b3)
        x[4,-1] = Gmk * (np.abs(5 - (5 + -1)*k[4]/(1+k[4])))**(-n_2b3)
        x[5,-1] = Gmk * (np.abs(6 - (6 + -1)*k[5]/(1+k[5])))**(-n_2b3)
        x[6,-1] = Gmk * (np.abs(7 - (7 + -1)*k[6]/(1+k[6])))**(-n_2b3)
        ...
        et = ( x_3b2*(a_3b2) + x*(a_1) + x_5b2*(a_5b2) + x_2*(a_2) + a_0 + x_3*(a_3) )
        
        
        #####################################################################################
        global Cp0,Sp0,Cx0,Sx0,Cp05,Sp05,Cx05,Sx05,Cp1,Sp1,Cx1,Sx1
        ...
        #Xi here incorporates the effect of unitary function. 
        #For those (l,n) where unitary is zero, Xi(l,n) is zero 
        
        #frequency domain waveform 
        #remember psi( l-1 , n )
        psi = np.zeros((10,9))
        xi1_p = np.zeros((10,9))*1j 
        xi1_c = np.zeros((10,9))*1j
        xipn_p = np.zeros((10,9))*1j
        xipn_c = np.zeros((10,9))*1j
        xi0_p = np.zeros((10,9))*1j
        xi0_c = np.zeros((10,9))*1j
        xi05_p = np.zeros((10,9))*1j
        xi05_c = np.zeros((10,9))*1j
        
        ##########1PN#############
        # xi1( l-1 , n )
        #n = -2 
        #getting values for psi,xi1_p,xi1_c,xipn_p,xipn_c,xi0_p,xi_c
        psi[0,-2],xi1_p[0,-2],xi1_c[0,-2],xipn_p[0,-2],xipn_c[0,-2],xi0_p[0,-2],xi0_c[0,-2] = psi_xi1_xipn_xi0_pc(1,-2)
        psi[1,-2],xi1_p[1,-2],xi1_c[1,-2],xipn_p[1,-2],xipn_c[1,-2],xi0_p[1,-2],xi0_c[1,-2] = psi_xi1_xipn_xi0_pc(2,-2)
        psi[2,-2],xi1_p[2,-2],xi1_c[2,-2],xipn_p[2,-2],xipn_c[2,-2],xi0_p[2,-2],xi0_c[2,-2] = psi_xi1_xipn_xi0_pc(3,-2)
        psi[3,-2],xi1_p[3,-2],xi1_c[3,-2],xipn_p[3,-2],xipn_c[3,-2],xi0_p[3,-2],xi0_c[3,-2] = psi_xi1_xipn_xi0_pc(4,-2)
        psi[4,-2],xi1_p[4,-2],xi1_c[4,-2],xipn_p[4,-2],xipn_c[4,-2],xi0_p[4,-2],xi0_c[4,-2] = psi_xi1_xipn_xi0_pc(5,-2)
        psi[5,-2],xi1_p[5,-2],xi1_c[5,-2],xipn_p[5,-2],xipn_c[5,-2],xi0_p[5,-2],xi0_c[5,-2] = psi_xi1_xipn_xi0_pc(6,-2)
        psi[6,-2],xi1_p[6,-2],xi1_c[6,-2],xipn_p[6,-2],xipn_c[6,-2],xi0_p[6,-2],xi0_c[6,-2] = psi_xi1_xipn_xi0_pc(7,-2)
        psi[7,-2],xi1_p[7,-2],xi1_c[7,-2],xipn_p[7,-2],xipn_c[7,-2],xi0_p[7,-2],xi0_c[7,-2] = psi_xi1_xipn_xi0_pc(8,-2)
        ...
        #n = 0
        psi[0,0],xi1_p[0,0],xi1_c[0,0],xipn_p[0,0],xipn_c[0,0],xi0_p[0,0],xi0_c[0,0] = psi_xi1_xipn_xi0_pc(1,0)
        psi[1,0],xi1_p[1,0],xi1_c[1,0],xipn_p[1,0],xipn_c[1,0],xi0_p[1,0],xi0_c[1,0] = psi_xi1_xipn_xi0_pc(2,0)
        psi[2,0],xi1_p[2,0],xi1_c[2,0],xipn_p[2,0],xipn_c[2,0],xi0_p[2,0],xi0_c[2,0] = psi_xi1_xipn_xi0_pc(3,0)
        psi[3,0],xi1_p[3,0],xi1_c[3,0],xipn_p[3,0],xipn_c[3,0],xi0_p[3,0],xi0_c[3,0] = psi_xi1_xipn_xi0_pc(4,0)
        psi[4,0],xi1_p[4,0],xi1_c[4,0],xipn_p[4,0],xipn_c[4,0],xi0_p[4,0],xi0_c[4,0] = psi_xi1_xipn_xi0_pc(5,0)
        psi[5,0],xi1_p[5,0],xi1_c[5,0],xipn_p[5,0],xipn_c[5,0],xi0_p[5,0],xi0_c[5,0] = psi_xi1_xipn_xi0_pc(6,0)        
        ...
        #n = -4 
        psi[0,-4],xi1_p[0,-4],xi1_c[0,-4] = psi_xi1_pc(1,-4)
        psi[1,-4],xi1_p[1,-4],xi1_c[1,-4] = psi_xi1_pc(2,-4)
        psi[2,-4],xi1_p[2,-4],xi1_c[2,-4] = psi_xi1_pc(3,-4)
        psi[3,-4],xi1_p[3,-4],xi1_c[3,-4] = psi_xi1_pc(4,-4)
        psi[4,-4],xi1_p[4,-4],xi1_c[4,-4] = psi_xi1_pc(5,-4)
        psi[5,-4],xi1_p[5,-4],xi1_c[5,-4] = psi_xi1_pc(6,-4)
        psi[6,-4],xi1_p[6,-4],xi1_c[6,-4] = psi_xi1_pc(7,-4)
        psi[7,-4],xi1_p[7,-4],xi1_c[7,-4] = psi_xi1_pc(8,-4)
        psi[8,-4],xi1_p[8,-4],xi1_c[8,-4] = psi_xi1_pc(9,-4)
        psi[9,-4],xi1_p[9,-4],xi1_c[9,-4] = psi_xi1_pc(10,-4)
        ...
        #n = 0  
        L1_Psi_N0_0 = ((n_1b2)**(n_2b3))*Psi_N0_0
        L2_Psi_N0_1 = ((n_2b2)**(n_2b3))*Psi_N0_1
        L3_Psi_N0_2 = ((n_3b2)**(n_2b3))*Psi_N0_2
        L4_Psi_N0_3 = ((n_4b2)**(n_2b3))*Psi_N0_3
        L5_Psi_N0_4 = ((n_5b2)**(n_2b3))*Psi_N0_4
        L6_Psi_N0_5 = ((n_6b2)**(n_2b3))*Psi_N0_5
        ...
        # xi05_p[ l-1 , n ]
        #n = -1
        psi[0,-1],xi05_p[0,-1],xi05_c[0,-1] = psi_xi05_pc(1,-1)
        psi[1,-1],xi05_p[1,-1],xi05_c[1,-1] = psi_xi05_pc(2,-1)
        psi[2,-1],xi05_p[2,-1],xi05_c[2,-1] = psi_xi05_pc(3,-1)
        psi[3,-1],xi05_p[3,-1],xi05_c[3,-1] = psi_xi05_pc(4,-1)
        psi[4,-1],xi05_p[4,-1],xi05_c[4,-1] = psi_xi05_pc(5,-1)
        psi[5,-1],xi05_p[5,-1],xi05_c[5,-1] = psi_xi05_pc(6,-1)
        psi[6,-1],xi05_p[6,-1],xi05_c[6,-1] = psi_xi05_pc(7,-1)
        ...
        #n = -3   
        psi[0,-3],xi05_p[0,-3],xi05_c[0,-3] = psi_xi05_pc(1,-3)
        psi[1,-3],xi05_p[1,-3],xi05_c[1,-3] = psi_xi05_pc(2,-3)
        psi[2,-3],xi05_p[2,-3],xi05_c[2,-3] = psi_xi05_pc(3,-3)
        psi[3,-3],xi05_p[3,-3],xi05_c[3,-3] = psi_xi05_pc(4,-3)
        psi[4,-3],xi05_p[4,-3],xi05_c[4,-3] = psi_xi05_pc(5,-3)
        psi[5,-3],xi05_p[5,-3],xi05_c[5,-3] = psi_xi05_pc(6,-3)
        psi[6,-3],xi05_p[6,-3],xi05_c[6,-3] = psi_xi05_pc(7,-3)
        psi[7,-3],xi05_p[7,-3],xi05_c[7,-3] = psi_xi05_pc(8,-3)
        psi[8,-3],xi05_p[8,-3],xi05_c[8,-3] = psi_xi05_pc(9,-3)
        ...
        # final frequency domain strain, plus polarization
        ...
        #only xi value is different from the calculation in plus polarisation  
        ...
        # final frequency domain strain, cross polarization
...
#######################################################################################        
    #this function gives value for psi, xi1, xipn and xi0 for n=-2,2,0
    def psi_xi1_xipn_xi0_pc_(self, l , n ):
        ...
*log_2)/419198976 + (130622307075*log_3)/41402368 + (249956266625*log_x_)/279465984 - (249956266625*log_chi)/419198976) + 1/chi_37b9*(1720927919854684009084595897/40516888294827538513920 - (3986831179520597*Pi_**2)/8405059239936 - (1423526912698421*gamma)/255362042880 + (131543151853096063653535/6190510052685643776 - (775201866281389*Pi_**2)/466947735552)*eta - (174156473319672237061*eta_2)/96372409245106176 + (18782995537481836405*eta_3)/5162807638130688 - (233279096767651103*log_2)/766086128640 + (773344207011339*log_3)/4450754560 - (164052734375*log_5)/20959232 - (1423526912698421*log_x_)/510724085760 + (1423526912698421*log_chi)/766086128640)))))

            et_ = et[l-1,n] 
            ###############Xi 1PN plus polarization####################
            Gamma_l = Cp1[l-1,n]
            Sigma_l = Sp1[l-1,n]
            ...
            #Xi1 1PN
            numerator = (1-et_**2)**(n_7b4)
            denomitor = ( 1 + (73/24)*et_**2 + (37/96)*et_**4 )**(n_1b2)
            xi1_p = (numerator/denomitor)*al*np.exp(-1j*phil)

            ###############Xi 1PN cross polarization####################
            Gamma_l = Cx1[l-1,n]
            Sigma_l = Sx1[l-1,n]
            ...
            #Xi1 1PN
            #numerator and denominator already define above
            xi1_c = (numerator/denomitor)*al*np.exp(-1j*phil)

            ###############Xipn and Xi0 plus polarization####################
            Gamma_l = Cp0[l-1,n]
            Sigma_l = Sp0[l-1,n]
            ...
            #xipn 1PN
            numerator_pn = (1-et_**2)**(3/4)*(11888 + 14784*eta - et_**2*(87720-159600*eta) - et_**4*(171038-141708*eta) - et_**6*(11717-8288*eta))
            denomitor_pn = ( 1 + (73/24)*et_**2 + (37/96)*et_**4 )**(n_3b2) * 10752

            xipn_p = (numerator_pn/denomitor_pn)*al*np.exp(-1j*phil)

            #Xi0 0PN
            #numerator and denominator already define above in 1PN
            xi0_p = (numerator/denomitor)*al*np.exp(-1j*phil)

            ###############Xipn and Xi0 cross polarization####################
            Gamma_l = Cx0[l-1,n]
            Sigma_l = Sx0[l-1,n]
            ...
            #xipn 1PN
            #numerator_pn and denominator_pn already define above
            xipn_c = (numerator_pn/denomitor_pn)*al*np.exp(-1j*phil)

            #Xi0 0PN
            #numerator and denominator already define above
            xi0_c = (numerator/denomitor)*al*np.exp(-1j*phil)
        
        return(psi,xi1_p,xi1_c,xipn_p,xipn_c,xi0_p,xi0_c)
    
    
    
#################################################################################
    #this function gives value for psi, xi1 n=-4,4
    def psi_xi1_pc_(self, l , n ):
        ...
        return(psi,xi1_p,xi1_c)
    
    
#################################################################################
    #this function gives value for psi, xi05 n=-1,1,-3,3
    def psi_xi05_pc_(self, l , n ):
        ...
        if unit[l-1,n]==0:
            ...
            numerator = (1-et_**2)**(n_7b4)
            denomitor = ( 1 + (73/24)*et_**2 + (37/96)*et_**4 )**(n_1b2)
            xi05_p = (numerator/denomitor)*al*np.exp(-1j*phil)

            ###############Xi 05PN cross polarization####################
            Gamma_l = Cx05[l-1,n]
            Sigma_l = Sx05[l-1,n]
            ...
            #Xi1 1PN
            #numerator and denominator already define above
            xi05_c = (numerator/denomitor)*al*np.exp(-1j*phil)
        
        return(psi,xi05_p,xi05_c)    
