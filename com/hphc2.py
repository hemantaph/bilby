...
def binomial_array(n, k):
    # n is a number and k is an 1d array
    z = np.zeros(len(k))
    n = n + z
    nk = n - k
    
    z[(n>=0)&(k>=0)&(nk>=0)] = factorial( n[(n>=0)&(k>=0)&(nk>=0)] )/( factorial( k[(n>=0)&(k>=0)&(nk>=0)] )*\
                                                              factorial( nk[(n>=0)&(k>=0)&(nk>=0)] ) )
    return(z)

# here l, m, the, phi are all number not array
# I can pass an array in a function
def Y(l, m, THE, PHI):
    
    k = np.arange(max(0,m-s),l-s+1)
    # conditions to find the binomial term
    condi1 = binomial_array( l-s , k )
    condi2 = binomial_array( l+s , k+(s-m) )
    
    # hlp is an 1D array with elements=0 if condi1=0 or condi2=2  
    hlp = (-1)**(l-k-s+m) * condi1 * condi2 * \
            cos(THE/2)**(2*k+s-m) * sin(THE/2)**(2*l-2*k-s+m) * e**(1j*m*PHI)
    
    return( sqrt(factorial(l+m)*factorial(l-m)*(2*l+1)/(4*pi*factorial(l+s)*factorial(l-s)))*hlp.sum() )
