...
# Define the waveform
def WaVe(surrogate,M, q, dis, the, phi, v_mag, v_the, v_phi, times):

    M = M*M_sun
    dis = dis*Mpc
    the = pi*float(the)/180
    phi = pi*float(phi)/180
    v_mag = 1000*float(v_mag)/c
    v_the = pi*float(v_the)/180
    v_phi = pi*float(v_phi)/180
